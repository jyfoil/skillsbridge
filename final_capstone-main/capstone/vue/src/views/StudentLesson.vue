<template>
  <div class="dashboard">
    <div id="heading-bg">
      ...
      <div v-if="lesson.has_assignment" class="heading-info mt-0">
        Assignment Due Date: {{ lesson.dueDate }}
      </div>
      <h3 class="dashboard-title">
        {{ course.name }} - <span class="course-name">{{ module.name }}</span>
      </h3>
      <router-link
        class="return-button"
        tag="button"
        :to="{ name: 'student-module', props: { courseId: module.courseId } }"
        ><img class="icon invert" src="../assets/arrow_back.svg" /> Back to
        Module</router-link
      >
    </div>
      ...
      <div id="content">
        ...
        <div v-if="lesson.has_assignment">
          ...
          <div class="assignment-details">
            <p>
              <strong>Instructions:</strong> {{ lesson.instructions }} <br />
              <strong>Due Date:</strong> {{ lesson.dueDate }}
            </p>
          </div>
          <h3>Submission</h3>
          <div
        ...
        <div class="assignment-details mt-2" v-else>
        ...
        <router-link
          ...
          :to="{
            name: 'student-module',
            props: { courseId: module.courseId, moduleId: module.id },
          }"
          ><img class="icon invert" src="../assets/arrow_back.svg" />Back to
          Module</router-link
        >
...
export default {
  ...
  created: function() {
    lessonService.getLesson(this.$route.params.lessonId).then((response) => {
      ...
      moduleService
        .getModule(this.$route.params.courseId, this.$route.params.moduleId)
        .then((response) => {
          if (response.status === 200) {
